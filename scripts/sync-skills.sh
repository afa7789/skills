...
#   OMP      — skills verbatim under ~/.omp/agent/, agents translated to OMP's
#              lowercase tool names with `model:` dropped so the session's
#              modelRoles pick the model. OMP's own config.yml, AGENTS.md and
#              models.yml are the user's files and are never touched.
#
...
#   OMP_HOME                    override ~/.omp (default ~/.omp)
...
ALL_TOOLS="claude opencode codex hermes pi omp"
...
tool_installed() {
    case "$1" in
        ...
        omp)      [ -d "$OMP_AGENT_HOME" ] || command -v omp     >/dev/null 2>&1 ;;
        *)        return 1 ;;
    esac
}
...
# 16. Sync: OMP target
# =============================================================================
# OMP reads native config from ~/.omp/agent/:
#   skills/<name>/SKILL.md  — same SKILL.md format as Claude/Pi
#   agents/<name>.md        — task-agent definitions (name/description/tools/
#                             model frontmatter; `mode:` is not part of OMP's
#                             schema and `tools:` uses OMP's lowercase ids)
#
# ~/.omp/agent/config.yml, AGENTS.md and models.yml belong to the user (roles,
# provider credentials, personal context) and are never written here.

# Claude tool names → OMP built-in ids (see src/tools/builtin-names.ts).
# Aliases OMP already canonicalizes (`Find`→glob, `Search`→grep) are mapped
# here too, so the emitted frontmatter is canonical. Anything without an OMP
# equivalent is dropped rather than guessed at.
translate_claude_agent_to_omp() {
    awk '
        BEGIN { fm = 0 }
        ...
        fm == 1 && $0 == "---" {
            if (csv != "") {
                ...
                for (i = 1; i <= n; i++) {
                    gsub(/^[[:space:]]+|[[:space:]]+$/, "", parts[i])
                    t = tolower(parts[i])
                    if      (t == "read")       map["read"]  = 1
                    else if (t == "edit")       map["edit"]  = 1
                    else if (t == "write")      map["write"] = 1
                    else if (t == "grep")       map["grep"]  = 1
                    else if (t == "bash")       map["bash"]  = 1
                    else if (t == "glob")       map["glob"]  = 1
                    else if (t == "task")       map["task"]  = 1
                    else if (t == "agent")      map["task"]  = 1
                    else if (t == "websearch")  map["web_search"] = 1
                    else if (t == "webfetch")   map["web_search"] = 1
                    else if (t == "todowrite")  map["todo"]  = 1
                    else if (t == "lsp")        map["lsp"]   = 1
                }
                m = split("read bash edit write glob grep lsp task todo web_search", ord, " ")
                mapped = ""
                for (i = 1; i <= m; i++)
                    if (map[ord[i]])
                        mapped = mapped (mapped == "" ? "" : ", ") ord[i]
        ...
        fm == 1 {
            ...
            # OMP resolves the model from session modelRoles; a bare Claude
            # alias (sonnet/opus/haiku) is not an OMP selector, and a pinned
            # model here would override the user role configuration.
            if ($0 ~ /^model:/) next
            # `mode:` is an OpenCode concept; OMP has no primary/subagent split
            # for user agent files.
            if ($0 ~ /^mode:/)  next
            print
            next
        }
...
step_sync_omp() {
    target_active omp || return 0

    local key="sync.omp" label="sync:omp" cur
    cur="$(source_sha)"
    step_begin "$key" "$cur" "$label" || return 0

    if want_content; then
        ensure_dir "$OMP_AGENT_HOME/agents" "$OMP_AGENT_HOME/skills"

        local f
        for f in "$PROJECT_ROOT/agents/"*.md; do
            translate_claude_agent_to_omp "$f" \
                > "$OMP_AGENT_HOME/agents/$(basename "$f")"
        done

        copy_skills_into "$OMP_AGENT_HOME/skills"
        prune_stale_agent_skills "$OMP_AGENT_HOME/skills"
    fi
...
# =============================================================================
# 17. Step: per-project AGENTS.md (opt-in)
# =============================================================================
